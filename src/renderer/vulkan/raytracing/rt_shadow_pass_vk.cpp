...
RTShadowPassVk::RTShadowPassVk(
	...
	sbt_(vk)
{
	factor_ = std::max(1u, rs_.resScale.RT_SHADOW);
...
RTShadowPassVk::~RTShadowPassVk() = default;

void RTShadowPassVk::init()
{
	...
	shader_ = std::make_unique<RayTracingShaderModuleVk>(
		vk_.getDevice(),
		"raytracing/rt_shadows/rt_shadow_raygen.rgen.spv",
		"raytracing/rt_shadows/rt_shadow_miss.rmiss.spv",
		std::vector<HitGroupFilePath>{
			{
				"",
				"raytracing/rt_shadows/rt_shadow_anyhit.rahit.spv"
			}
	}
	);
...
)
{
	...
	cmd.beginDebugUtilsLabelEXT({ "RTShadowPassVk::cmd" });
...
//--- PRIVATE ---//
void RTShadowPassVk::syncSettings()
{
	uint32_t newFactor = std::max(1u, rs_.resScale.RT_SHADOW);
	...
	// RAYGEN SET
	{
		...
		if (tlas_[frameIndex].valid())
		{
			set.writeAccelerationStructure(
				TO_API_FORM(RTShadowRayGenBinding::TLAS),
		...
		if (normalTex_ && normalTex_->valid())
		{
			set.writeCombinedImageSampler(
				TO_API_FORM(RTShadowRayGenBinding::NormalTex),
				normalTex_->view(),
				normalTex_->sampler()
			);
		}
		...
		{
			set.writeCombinedImageSampler(
				TO_API_FORM(RTShadowRayGenBinding::DepthTex),
				depthTex_->view(),
				depthTex_->sampler()
			);
		}
...
void RTShadowPassVk::createResources()
{
	for (uint32_t i = 0; i < vk_.getMaxFramesInFlight(); ++i)
	{
...
void RTShadowPassVk::createDescriptorSet()
{
	for (uint32_t i = 0; i < vk_.getMaxFramesInFlight(); ++i)
	{
		// RAYGEN DS + POOL
		{
			...
			outColorBinding.binding = TO_API_FORM(RTShadowRayGenBinding::OutColorImage);
			...
			tlasBinding.binding = TO_API_FORM(RTShadowRayGenBinding::TLAS);
			...
			uboBinding.binding = TO_API_FORM(RTShadowRayGenBinding::UBO);
			uboBinding.descriptorType = vk::DescriptorType::eUniformBuffer;
			uboBinding.descriptorCount = 1;
			uboBinding.stageFlags = vk::ShaderStageFlagBits::eRaygenKHR;
			
			vk::DescriptorSetLayoutBinding normalTexBinding{};
			normalTexBinding.binding = TO_API_FORM(RTShadowRayGenBinding::NormalTex);
			...
			depthTexBinding.binding = TO_API_FORM(RTShadowRayGenBinding::DepthTex);
			...
			rayGenDescriptorSets_[i].setDebugName(
				"RTShadowPassVk-RayGen::DescriptorSet frame " + std::to_string(i)
			);
		}
	} // end for
} // end of createDescriptorSet()

void RTShadowPassVk::createPipeline()
{
	...
	pipeline_.setDebugName("RTShadowPassVk::Pipeline");
} // end of createPipeline()

void RTShadowPassVk::createSBT()
{
	sbt_.create(
		pipeline_.getPipeline(),
		3,
